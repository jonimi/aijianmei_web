	...
	_this.parseShow = function(data,position){
		...
		if(isLogin) {
			...
			if( data['follow_state'] =='unfollow' ){
				...
				$('#space_card_toolbar').html('<span>已关注 | <a href="javascript:;" onclick="space_card_dofollow(\'unflollow\','+uid+',\''+data['uname']+'\')">取消</a></span>');
			}else if(data['follow_state'] =='eachfollow'){
				$('#space_card_toolbar').html('<span>互相关注 | <a href="javascript:;" onclick="space_card_dofollow(\'unflollow\','+uid+',\''+data['uname']+'\')">取消</a></span>');
			}
		}
	}
};
...
function space_card_dofollow(type,uid,username){
	...
	$.post( U('weibo/Operate/follow') ,{uid:uid,type:type},function(txt){
		if(txt=='12'){
			$('#space_card_toolbar').html('<span>已关注 | <a href="javascript:;" onclick="space_card_dofollow(\'unflollow\','+uid+',\''+username+'\')">取消</a></span>');
			//userInfoList[uid]['follow_state'] = 'havefollow';
				followGroupSelectorBox(uid);
		}else if(txt=='13'){
			$('#space_card_toolbar').html('<span>互相关注 | <a href="javascript:;" onclick="space_card_dofollow(\'unflollow\','+uid+',\''+username+'\')">取消</a></span>');
