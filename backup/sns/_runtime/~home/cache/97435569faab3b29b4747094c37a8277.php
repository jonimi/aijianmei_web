...
<div class="content"><!-- 内容 begin  --> 
  <div class="main no_l">
    ...
    <div class="reg_process">
        <div class="reg_process_def"><div>1.填写基本信息</div></div>
        <div class="reg_process_on"><div>2.看看推荐的人</div></div>
        <div class="reg_process_def"><div>3.完成，进入我的首页</div></div>
        </div>
    </div>
    ...
    <div class="mainbox_appC boxspace_L"> 
        <div class="reg_box">
        <h2 class="cBlue"><strong style="font-size:45px">02.</strong>关注你感兴趣的人，分享新鲜事</h2>
        <p class="cGray lh20" style="padding:10px 0">关注其它人才能收到TA们分享的新鲜事。关注的人越多，交流越精彩。</p>
        <h2 class="f14px lh35"> 人气用户推荐：TA们正在被很多人关注，也许是你感兴趣的朋友哦。</h2>
        ...
        <div class="feedBox">
          <ul class="feed_list">
           <?php if(is_array($users)): ?><?php $i = 0;?><?php $__LIST__ = $users?><?php if( count($__LIST__)==0 ) : echo "" ; ?><?php else: ?><?php foreach($__LIST__ as $key=>$vo): ?><?php ++$i;?><?php $mod = ($i % 2 )?><li>
            <div class="left" style="width:25px; padding-top:20px;"><input name="followuid[]" type="checkbox" checked="checked" value="<?php echo ($vo["uid"]); ?>" /></div>
              <div class="userPic"><a title="" href="<?php echo U('home/space/index',array('uid'=>$vo['uid']));?>" target="_blank"><img src="<?php echo (getUserFace($vo["uid"],'m')); ?>" card="1"></a></div>
              <div style="margin-left: 95px;" class="feed_c">
                <div style="padding-bottom: 0pt;">
                  <a href="<?php echo U('home/space/index',array('uid'=>$vo['uid']));?>" target="_blank"><?php echo ($vo["uname"]); ?></a> (<?php echo ($vo["count"]); ?>人关注)
                </div>
          ...
          <div  class="pt10 clearL"><span class="right"><a href="<?php echo U('home/user/index');?>">跳过&gt;&gt; </a></span>
            <input name="" onclick="checkAllFollow(this)" type="checkbox" /> 
           <input name="确定" type="submit" class="btn_green_w121" value="关注所选用户" /></div>
        </div>
...
<script>
	function checkAllFollow(o){
		if( o.checked == true ){
			$("input[name='followuid[]']").attr('checked',true);
		}else{
