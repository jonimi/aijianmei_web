    ...
    <div class="mainbox">
      <div class="mainbox_C no_r boxMessage">
        ...
        <div style=" overflow: hidden; width: 100%;">
          <ul class="feed_list">
          	<?php if(is_array($data)): ?><?php $i = 0;?><?php $__LIST__ = $data?><?php if( count($__LIST__)==0 ) : echo "" ; ?><?php else: ?><?php foreach($__LIST__ as $key=>$vo): ?><?php ++$i;?><?php $mod = ($i % 2 )?><li class="lineD_btm" id="notify_<?php echo ($vo["notify_id"]); ?>">
              <?php if(($vo['from'])  !=  "0"): ?><div class="userPic"><?php echo getUserSpace($vo['from'],'','_blank','{uavatar=m}') ?></div><?php endif; ?>

              <div class="feed_c hover">
                <h3>
                  <div class="right alR" style="line-height:20px">
                    <div class="left"><em><?php echo (friendlyDate($vo["ctime"])); ?></em></div>
                    <div class="left"><a style="margin-top: 2px" class="del" href="javascript:deleteNotify(<?php echo ($vo["notify_id"]); ?>)" title="删除"></a></div>
                  </div>
                  <span class="lh20 f12px">
                    <?php if(($vo['type'])  ==  "gift_send"): ?><?php if(($vo['from'])  ==  "0"): ?>神秘人物<?php endif; ?><?php endif; ?>
                    <?php echo ($vo["title"]); ?></span></h3>
                <div class="feed_c_btm">
                  <?php echo ($vo["body"]); ?>
                </div>
				<div style="margin-top: 15px;"> <?php echo ($vo["other"]); ?> </div>
                <?php if(false && in_array($vo['type'], array('home_addComment','home_replyComment'))) { ?>
				<div class="feed_quote feed_wb" style="display:none;">
                    ...
                    <form method="post" action="">
                        <input type="hidden" name="notify_id" value="<?php echo ($vo['notify_id']); ?>">
