...
$(document).ready(function(){
    $("tr[overstyle='on']").hover(
      function () {
        $(this).addClass("iine_warning");
      },
      function () {
        $(this).removeClass("iine_warning");
      }
    );
});
</script>
		...
		<div class="setupBox" style="margin-right:0px;">
          ...
          <div class="setItems" style="_width:100%">
            <table width="100%" border="0" cellspacing="0" cellpadding="0" style="line-height:25px;">
  			  <tr>
  			  	<th class="lineD_btm cGray2">动作</th>
  			  	<?php if(is_array($credit_type)): ?><?php $i = 0;?><?php $__LIST__ = $credit_type?><?php if( count($__LIST__)==0 ) : echo "" ; ?><?php else: ?><?php foreach($__LIST__ as $key=>$ct): ?><?php ++$i;?><?php $mod = ($i % 2 )?><th class="lineD_btm cGray2" style="text-align:center"><?php echo ($ct["alias"]); ?></th><?php endforeach; ?><?php endif; ?><?php else: echo "" ;?><?php endif; ?>
		   	  </tr>
		   	  <?php if(is_array($credit_rules)): ?><?php $i = 0;?><?php $__LIST__ = $credit_rules?><?php if( count($__LIST__)==0 ) : echo "" ; ?><?php else: ?><?php foreach($__LIST__ as $key=>$vo): ?><?php ++$i;?><?php $mod = ($i % 2 )?><tr overstyle='on' >
		   	  	<td class="lineS_btm"><?php echo ($vo["alias"]); ?></td>
		   	  	<?php if(is_array($credit_type)): ?><?php $i = 0;?><?php $__LIST__ = $credit_type?><?php if( count($__LIST__)==0 ) : echo "" ; ?><?php else: ?><?php foreach($__LIST__ as $key=>$ct): ?><?php ++$i;?><?php $mod = ($i % 2 )?><th class="lineS_btm" style="text-align:center"><?php echo ($vo[$ct['name']]); ?></th><?php endforeach; ?><?php endif; ?><?php else: echo "" ;?><?php endif; ?>
