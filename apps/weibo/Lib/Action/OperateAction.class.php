...
class OperateAction extends Action{
    ...
    function publish(){
        ...
        if (isDuplicateContent(trim($_POST['content'] . $_POST['publish_type_data']))) {
    ...
    function addcomment(){
        ...
        if(t($_POST['comment_content']) == ''){
            $this->assign('评论内容不能为空');
        }
		if(isSubmitLocked()){
			die('submitlocked');
    ...
    function deleteMuleComment(){
        ...
        $res = M('weibo_comment')->where($map)->delete();
    	$result = D('Comment')->deleteMuleComments( $_POST['id'] ,$_POST['commentid'], $this->mid);
    	echo json_encode($result);
    }
    ...
    function quickpublish(){
    	$this->assign('text', t($_REQUEST['text']) );
    	$this->display();
    }
