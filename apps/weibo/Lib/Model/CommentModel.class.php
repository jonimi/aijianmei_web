    ...
    public function doaddcomment($uid, $post, $api=false)
    {
        ...
        if ( $comment_id = $this->addcomment( $data ) ){
            if($data['uid'] == $data['reply_uid']){
                X('Credit')->setUserCredit($data['uid'],'add_comment');
            }
			//微博回复积分操作
			if($data['uid'] != $data['reply_uid']){
				X('Credit')->setUserCredit($data['uid'],'reply_comment');
			    X('Credit')->setUserCredit($data['reply_uid'],'replied_comment');
                X('Credit')->setUserCredit($data['reply_uid'],'is_commented');
			}
