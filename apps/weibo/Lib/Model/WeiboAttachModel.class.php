...
class WeiboAttachModel extends Model{
	...
	public function getWeiboAlbum($uid){
		...
		//获取最后一个图片微博
		if($count > 0){
			$last_weibo = $this->where("weibo_type=1 AND uid=".$uid)->order('attach_id desc')->limit(1)->find();
		...
		$weibo  =  array(
			  ...
			  "name" => "微博相册",
			  "info" => NULL,
			  "cTime" =>  $last_weibo_attach['uploadTime']?$last_weibo_attach['uploadTime']:time(),
			  "mTime" =>  $last_weibo_attach['uploadTime']?$last_weibo_attach['uploadTime']:time(),
