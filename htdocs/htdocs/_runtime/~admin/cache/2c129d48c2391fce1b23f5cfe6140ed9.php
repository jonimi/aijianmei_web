...
<div class="so_main">
  ...
  <div class="Toolbar_inbox">
	<a href="<?php echo U('admin/Article/add');?>" class="btn_a"><span>添加文章</span></a>
    <a href="javascript:void(0);" class="btn_a" onclick="deleteAd();"><span>删除文章</span></a>
  </div>
  <div class="list">
  ...
  <tr>
    ...
    <th class="line_l">所属分类</th>
    <th class="line_l">操作</th>
  </tr>
  <?php if(is_array($article)): ?><?php $i = 0;?><?php $__LIST__ = $article?><?php if( count($__LIST__)==0 ) : echo "" ; ?><?php else: ?><?php foreach($__LIST__ as $key=>$vo): ?><?php ++$i;?><?php $mod = ($i % 2 )?><tr overstyle='on' id="<?php echo ($vo["id"]); ?>">
        ...
        <td>
			<a href="<?php echo U('admin/Article/edit',array('id'=>$vo['id']));?>">编辑</a>
...
<script>
    ...
    function deleteAd(ids) {
    	...
    	if(confirm('您将删除'+length+'条记录，删除后无法恢复，确定继续？')) {
    		$.post("<?php echo U('admin/Article/doDeleteArticle');?>",{ids:ids},function(res){
    			if(res=='1') {
    				ui.success('删除成功');
    				removeItem(ids);
    			}else {
