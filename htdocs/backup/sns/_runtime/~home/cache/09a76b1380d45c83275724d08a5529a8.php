            ...
            <div class="setupBox" style="margin-right:0px;">
              ...
              <div class="setItems" style="_width:100%">
                <div class="setFold setUnfold" rel="email" >
                  <h2><img src="__THEME__/images/icon/ico_mail_ok.gif"  width="24" height="24" class="alM" /> 邮箱绑定 (<?php if($email){ ?><span class="f12px fn">已绑定</span><?php }else{ ?><span class="f12px fn cRed">未绑定</span><?php } ?>)</h2>
                </div>
                  ...
                  <div class="data"><!-- 修改密码 begin  -->
                    <ul>
                      ...
                      <div class="left" style="width: 50%;">
                        <div class="left mr5"><?php echo ($email); ?></div>
                      </div>
                      </li>
                      <?php }else{ ?>
                      <li>
              ...
              <?php if(is_array($bind)): ?><?php $i = 0;?><?php $__LIST__ = $bind?><?php if( count($__LIST__)==0 ) : echo "" ; ?><?php else: ?><?php foreach($__LIST__ as $key=>$vo): ?><?php ++$i;?><?php $mod = ($i % 2 )?><?php if($vo['isBind']){
                    $bindInfo = '已绑定';
                  }else{
                    $bindInfo = '未绑定';
                  } ?>
              <div class="setItems" style="_width:100%">
                <div class="setFold" rel="<?php echo ($vo["type"]); ?>" >
                  <h2><img src="<?php echo ($vo["ico"]); ?>" width="24" height="24"  class="alM"/> <?php echo ($vo["name"]); ?>(<?php echo ($bindInfo); ?>)</h2>
                </div>
                <div  class="setItemsInfo">
                  <div class="data"><!-- 新浪微博绑定 begin  -->
                    <?php if($vo['isBind']){ ?>
                    <ul>
                      <li>您已绑定<?php echo ($vo["name"]); ?></li>
                      <li><input type="button" class="btn_b" value="取消绑定" onclick="unbind1('<?php echo ($vo["name"]); ?>','<?php echo ($vo["type"]); ?>','<?php echo ($vo["addon"]); ?>','<?php echo ($vo["unbind_hook"]); ?>');" /></li>
                    </ul>
                    <?php }else{ ?>
                    <ul>
                      <li>您未绑定<?php echo ($vo["name"]); ?></li>
                      <li><a class="btn_b" href="<?php echo Addons::createAddonShow($vo['addon'],$vo['bind_hook'],array('type'=>$vo['type'],'do'=>'bind'));?>">开始绑定</a></li>
                    </ul>
                    <?php } ?>  
                  </div><!-- 新浪微博绑定 end  -->
                </div>
              ...
              <?php echo Addons::hook('home_account_bind_bottom');?>
              <div class="c"></div>
            </div>
          </div>
        </div>
