    ...
    <div class="mainbox">
      <div class="mainbox_C no_r boxMessage">
        ...
        <div style="width:100%; overflow:hidden;">
          ...
          <?php } ?>
         <ul class="feed_list">
          	<?php if(is_array($data)): ?><?php $i = 0;?><?php $__LIST__ = $data?><?php if( count($__LIST__)==0 ) : echo "" ; ?><?php else: ?><?php foreach($__LIST__ as $key=>$vo): ?><?php ++$i;?><?php $mod = ($i % 2 )?><li class="lineD_btm" id="myinvite_<?php echo ($vo["hash"]); ?>">
                <div class="userPic">
                    <a href="<?php echo U('home/space/index',array('uid'=>$vo['fromuid']));?>" title="">
                        <img card="1" src="<?php echo (getUserFace($vo["fromuid"])); ?>">
                    </a>
                </div>
                <div style="margin-left:70px;"><?php echo ($vo["myml"]); ?></div>
...
var weibo = $.weibo({
  timeStep : 60000
	...
	function manyou_add_userapp(hash, url){
        ...
        setTimeout(function(){
	        $.post( "<?php echo U('home/Message/deleteMyInvite');?>", {hash:hash}, function(res){
	            if (res == '1') {
	                if (url) {
	                    //同意
