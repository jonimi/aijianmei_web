...
<?php if(is_array($list)): ?><?php $i = 0;?><?php $__LIST__ = $list?><?php if( count($__LIST__)==0 ) : echo "" ; ?><?php else: ?><?php foreach($__LIST__ as $key=>$vo): ?><?php ++$i;?><?php $mod = ($i % 2 )?><?php if(($vo)  !=  ""): ?><li class="lineD_btm" id="list_li_<?php echo ($vo["weibo_id"]); ?>">
    ...
    <div class="feed_c" <?php if($hidden_user == 1): ?>style="margin-left:10px;"<?php endif; ?>>
      ...
      <div class="feed_c_btm"> 
          <span class="right">
            ...
            <a href="javascript:void(0)" onclick="$('.ts_confirm').remove();ui.confirm(this,'<?php echo L('del_confirm');?>')" callback="weibo.deleted(<?php echo ($vo["weibo_id"]); ?>)" style="color:red"><?php echo L('del');?></a>
            <i class="vline">|</i>
            <?php }elseif($vo['uid'] == $mid){ ?>
			<a href="javascript:void(0)" onclick="$('.ts_confirm').remove();ui.confirm(this,'<?php echo L('del_confirm');?>')" callback="weibo.deleted(<?php echo ($vo["weibo_id"]); ?>)"><?php echo L('del');?></a>
            <i class="vline">|</i>
			<?php } ?>
