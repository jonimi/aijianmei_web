...
<script type="text/javascript" src="__PUBLIC__/js/jquery.form.js"></script>
<!--<script type="text/javascript" src="<?php echo U('weibo/plugins/init');?>"></script>-->
...
<?php echo Addons::hook('home_space_right_bottom', array('uid'=>$uid));?>
  ...
  <dt><?php echo getUserSpace($uid,'','','{uavatar=b}') ?></dt>
		  ...
		  <?php echo Addons::hook('home_space_profile_middle', array($uid));?>
          <div class="friend_info_right">
            <div class="info_deta lineS_top">
                <dl class="lineD_top">
                	<dt>个人情况</dt>
					<?php if(is_array($userInfo["intro"]["list"])): ?><?php $i = 0;?><?php $__LIST__ = $userInfo["intro"]["list"]?><?php if( count($__LIST__)==0 ) : echo "" ; ?><?php else: ?><?php foreach($__LIST__ as $key=>$vo): ?><?php ++$i;?><?php $mod = ($i % 2 )?><?php if($vo['value']){ ?></php><dd><div class="c1"><?php echo ($vo["name"]); ?>:</div><div class="c2"><?php echo ($vo["value"]); ?></div></dd><?php } ?><?php endforeach; ?><?php endif; ?><?php else: echo "" ;?><?php endif; ?>
                </dl>
                <dl class="lineD_top">
                	<dt>联系方式</dt>
					<?php if(is_array($userInfo["contact"]["list"])): ?><?php $i = 0;?><?php $__LIST__ = $userInfo["contact"]["list"]?><?php if( count($__LIST__)==0 ) : echo "" ; ?><?php else: ?><?php foreach($__LIST__ as $key=>$vo): ?><?php ++$i;?><?php $mod = ($i % 2 )?><?php if($vo['value']){ ?></php><dd><div class="c1"><?php echo ($vo["name"]); ?>:</div><div class="c2"><?php echo ($vo["value"]); ?></div></dd><?php } ?><?php endforeach; ?><?php endif; ?><?php else: echo "" ;?><?php endif; ?>
                </dl>
                <?php if($userInfo['profile'] && $userInfo['profile']['list']){ ?>
                <dl class="lineD_top">
                  <dt>教育、工作情况</dt>
                  <?php if(is_array($userInfo["profile"]["list"])): ?><?php $i = 0;?><?php $__LIST__ = $userInfo["profile"]["list"]?><?php if( count($__LIST__)==0 ) : echo "" ; ?><?php else: ?><?php foreach($__LIST__ as $key=>$vo): ?><?php ++$i;?><?php $mod = ($i % 2 )?></php>
                      <dd>
                          <?php switch($vo['module']): ?><?php case "education":  ?><div class="projectlist">
                                      <h3>
                                      [教育]&nbsp;&nbsp;<a target="_blank"><?php echo ($vo["school"]); ?></a>
                                      </h3><p><?php echo ($vo["year"]); ?>入学 - <?php echo ($vo["classes"]); ?></p>
                                  </div><?php break;?>
                              <?php case "career":  ?><div class="projectlist">
                                      <h3>
          ...
          <div class="c"></div>
		  <?php echo Addons::hook('home_space_profile_bottom', array($uid));?>
      </div>
