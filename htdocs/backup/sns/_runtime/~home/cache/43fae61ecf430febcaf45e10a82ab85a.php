	...
	<div class="mainbox_appC no_r">
      <div class="mainbox_C no_r">
          ...
          <a href="<?php echo U('home/Index/editapp');?>" class="right mt10">管理应用</a>
            <ul>
              <li><a class="feed_item on" href="<?php echo U('home/Index/addapp');?>"><span>本地应用</span></a></li>
              <?php if($ts['site']['my_status']) { ?>
              <li><a class="feed_item " href="__ROOT__/apps/myop/myop.php?my_suffix=%2Fapp%2Flist"><span>漫游平台</span></a></li>
              <?php } ?>
            </ul>
          </div>
          <div class="add_app">
          <?php if(is_array($data)): ?><?php $i = 0;?><?php $__LIST__ = $data?><?php if( count($__LIST__)==0 ) : echo "" ; ?><?php else: ?><?php foreach($__LIST__ as $key=>$vo): ?><?php ++$i;?><?php $mod = ($i % 2 )?><dl class="add_app_list">
            ...
            <dt><img src="<?php echo getAppIconUrl($vo['large_icon_url'],$vo['app_name']);;?>" width="64px" height="64px"/></dt>
            <dd class="lineD_btm">
                <h3><a href="<?php echo U($vo['app_name'].'/'.$vo['app_entry']);?>"><?php echo ($vo['app_alias']); ?></a></h3>
                <p style="margin-right:100px;"><?php echo ($vo['description']); ?></p>
                <p class="cGray2">
	                <?php if(in_array($vo['app_id'],$installed) || $vo['status']==1) { ?>
	                <span class="right">已添加</span>
	                <?php }else { ?>
	                <a href="<?php echo U('home/Index/install',array('app_id'=>$vo['app_id']));?>" class="right">我要添加</a>
	                <?php } ?>
             <!--    作者: <a href="<?php echo ($vo['homepage_url']); ?>"><?php echo ($vo['author_name']); ?></a> &nbsp; -->
                    作者: <?php echo ($vo['author_name']); ?> &nbsp;
                    <?php if(!empty($vo['contributor_name'])) { ?>
                    贡献者:<?php echo ($vo['contributor_name']); ?>
                    <?php } ?>
                </p>
            </dd>
