...
<?php echo Addons::hook('home_space_right_bottom', array('uid'=>$uid));?>
  ...
  <dt><?php echo getUserSpace($uid,'','','{uavatar=b}') ?></dt>
      ...
      <div class="tab-menu mt10"><!-- 切换标签 begin  -->
        <ul>
          <li><a id="feed_all_item" rel="all" class="<?php if(($type)  ==  "following"): ?>on<?php endif; ?> feed_item" href="<?php echo U('home/space/follow',array('uid'=>$uid,'type'=>'following'));?>"><span>关注</span></a></li>
		 <li><a id="feed_colleague_item" rel="colleague" class="<?php if(($type)  ==  "follower"): ?>on<?php endif; ?> feed_item"  href="<?php echo U('home/space/follow',array('uid'=>$uid,'type'=>'follower'));?>"><span>粉丝</span></a></li>            
        </ul>
      </div>

      <!-- /个人主页分类添加 end -->
      <?php if($type == 'following'): ?><div class="MenuSub ">
        <?php if ($mid > 0 && $uid == $mid) { ?>
          <div class="right"><input type="button" onclick="setFollowGroupTab()" class="btn_w" value="创建分组" style=""></div>
        <?php } ?>
        <div class="left wn">
          <a href="<?php echo U('home/space/follow',array('uid'=>$uid,'type'=>'following'));?>" <?php if($gid == 'all'): ?>class="on"<?php endif; ?>>全部</a>
          <a href="<?php echo U('home/space/follow',array('uid'=>$uid,'type'=>'following','gid'=>0));?>" <?php if(!$gid): ?>class="on"<?php endif; ?>>未分组</a>
          <?php if(is_array($group_list_1)): ?><?php $i = 0;?><?php $__LIST__ = $group_list_1?><?php if( count($__LIST__)==0 ) : echo "" ; ?><?php else: ?><?php foreach($__LIST__ as $key=>$fg): ?><?php ++$i;?><?php $mod = ($i % 2 )?><a href="<?php echo U('home/space/follow',array('uid'=>$uid,'type'=>'following','gid'=>$fg['follow_group_id']));?>" <?php if($gid == $fg['follow_group_id']): ?><?php $gtitle=$fg['title']; ?>class="on" id="current_follow_group"<?php endif; ?>><?php echo ($fg['title']); ?></a><?php endforeach; ?><?php endif; ?><?php else: echo "" ;?><?php endif; ?>
	    </div>
		<?php if($group_list_2): ?><div onmouseout="this.className='feeds_sift'" onmouseover="this.className='feeds_sift_hover'" class="feeds_sift"><a href="#"><span >更多</span><span class="more"></span></a>
            <div class="feeds_sift_box">
              <span id="followGroup">
	            <?php if(is_array($group_list_2)): ?><?php $i = 0;?><?php $__LIST__ = $group_list_2?><?php if( count($__LIST__)==0 ) : echo "" ; ?><?php else: ?><?php foreach($__LIST__ as $key=>$fg): ?><?php ++$i;?><?php $mod = ($i % 2 )?><a href="<?php echo U('home/space/follow',array('uid'=>$uid,'type'=>'following','gid'=>$fg['follow_group_id']));?>" <?php if($gid == $fg['follow_group_id']): ?><?php $gtitle=$fg['title']; ?><?php endif; ?>><?php echo ($fg['title']); ?></a><?php endforeach; ?><?php endif; ?><?php else: echo "" ;?><?php endif; ?>
	          </span>
	        </div>
		  </div><?php endif; ?>
        </div><?php endif; ?>
        
      <?php if ($mid > 0 && $mid == $uid && intval($gid) > 0) { ?>
	  <div class="alR">
	  	<span class="pl5">共<?php echo ($list["count"]); ?>人</span> 
	  	<a href="javascript:void(0);" onclick="setFollowGroupTab('<?php echo ($gid); ?>')" ><span style="color:#FF0000;"></span>修改分组</a> 
	  	<a href="<?php echo U('weibo/FollowGroup/deleteGroup',array('gid'=>$gid));?>" onclick="if(confirm('确定要删除“<?php echo ($gtitle); ?>”分组吗？此分组下的人不会被取消关注。'))return ture;return false;" ><span style="color:#FF0000;">×</span>删除分组</a>
	  </div>
	  <?php } ?>
...
<?php if(is_array($list["data"])): ?><?php $i = 0;?><?php $__LIST__ = $list["data"]?><?php if( count($__LIST__)==0 ) : echo "" ; ?><?php else: ?><?php foreach($__LIST__ as $key=>$vo): ?><?php ++$i;?><?php $mod = ($i % 2 )?><li class="lineD_btm">
              ...
              <div class="feed_c" style="margin-left:75px">
				...
				<?php if ($mid > 0 && $vo['fid'] != $mid) { ?>
				<div class="right pr10" id="follow_list_<?php echo ($vo["fid"]); ?>" >
					<script>document.write(followState('<?php echo ($vo["followState"]); ?>','dolistfollow',<?php echo ($vo["fid"]); ?>))</script>
				</div>
				<?php } ?>
				<div style=" float:left; width:220px;"><?php echo getUserSpace($vo["fid"],'','','{uname}') ?><?php echo (getUserGroupIcon($vo["fid"])); ?></div>
				<?php if ($type == 'following' && $mid > 0 && $mid == $uid) { ?>
				  <?php echo W('FollowGroup',array('uid'=>$vo['uid'],'fid'=>$vo['fid']));?>
				<?php } ?>
				</div>

			    <div class="feed_c_btm lh18" style=" padding-top:5px; ">
		      <?php if($vo['mini']){ ?><div>最近微博 <?php echo (friendlyDate($vo["mini"]["ctime"])); ?> </div><?php } ?>
                    <div><a href="<?php echo U('home/space/detail',array('id'=>$vo['mini']['weibo_id']));?>"><?php echo ($vo["mini"]["content"]); ?></a></div>
                    <?php if($vo['user']['location']){ ?><div><?php echo ($vo["user"]["location"]); ?></div><?php } ?>
                    <div class="pt5"><span ><a href="<?php echo U('home/space/follow',array('type'=>'following','uid'=>$vo['fid']));?>">关注<strong><?php echo ($vo["following"]); ?></strong>人</a></span><span class="pl10"><a href="<?php echo U('home/space/follow',array('type'=>'follower','uid'=>$vo['fid']));?>">粉丝<strong><?php echo ($vo["follower"]); ?></strong>人</a></span></div>
				</div>
              </div>
