...
<div class="content_holder">
  <div class="content content_bg">
    ...
    <div class="main no_l mainC_pad">
        ...
        <?php if($user_list): ?>分类：
          <?php $j = 0; ?>
          <?php if(is_array($user_list)): ?><?php $i = 0;?><?php $__LIST__ = $user_list?><?php if( count($__LIST__)==0 ) : echo "" ; ?><?php else: ?><?php foreach($__LIST__ as $key=>$g): ?><?php ++$i;?><?php $mod = ($i % 2 )?><?php ++$j; ?>
            <?php if($j>1){ ?>&nbsp;|&nbsp;<?php }; ?><a href="#star_box_<?php echo ($g["star_group_id"]); ?>"><?php echo ($g["title"]); ?></a>
            <?php if(is_array($g["son_list"])): ?><?php $i = 0;?><?php $__LIST__ = $g["son_list"]?><?php if( count($__LIST__)==0 ) : echo "" ; ?><?php else: ?><?php foreach($__LIST__ as $key=>$s): ?><?php ++$i;?><?php $mod = ($i % 2 )?><?php if($i>1) ?>&nbsp;|&nbsp;<?php ; ?><a href="#star_box_<?php echo ($s["star_group_id"]); ?>"><?php echo ($s["title"]); ?></a><?php endforeach; ?><?php endif; ?><?php else: echo "" ;?><?php endif; ?><?php endforeach; ?><?php endif; ?><?php else: echo "" ;?><?php endif; ?><?php endif; ?>
          </div>
          <?php if(is_array($user_list)): ?><?php $i = 0;?><?php $__LIST__ = $user_list?><?php if( count($__LIST__)==0 ) : echo "" ; ?><?php else: ?><?php foreach($__LIST__ as $key=>$g): ?><?php ++$i;?><?php $mod = ($i % 2 )?><a name="star_box_<?php echo ($g["star_group_id"]); ?>"></a>
          <div id="star_box_<?php echo ($g["star_group_id"]); ?>" class="Share_box">
            <form action="<?php echo U('home/public/followuser');?>" method="post">
              <input type="hidden" name="doajax" value="1">
              <div class="topic_tit2">
                <?php if(($mid)  >  "0"): ?><span class="right" style="position:relative;padding:0 0 0 20px">
                      <?php if(count($g['user_list'])>$showCount){ ?>
                    <a class="changes" href="javascript:void(0)" star_target="list_<?php echo ($g["star_group_id"]); ?>">换一换</a>
                      <?php } ?>
                    <label>
                    <input type="checkbox" class="checkall checkbox1" cid="<?php echo ($g["star_group_id"]); ?>" value="b<?php echo ($g["star_group_id"]); ?>">
                    全选
                    </label>
                    <!--<input type="button" value="关注已选" class="btn_b" onclick="dofollow('#star_box_<?php echo ($g["star_group_id"]); ?>')" />-->
                     | <a href="javascript:void(0)" onclick="dofollow()">关注已选</a>
                    </span><?php endif; ?>
                <strong class="font_stroke"><?php echo ($g["title"]); ?></strong>
                </div>
              <ul class="list_square" id="list_<?php echo ($g["star_group_id"]); ?>">
                <?php if(is_array($g["user_list"])): ?><?php $i = 0;?><?php $__LIST__ = $g["user_list"]?><?php if( count($__LIST__)==0 ) : echo "" ; ?><?php else: ?><?php foreach($__LIST__ as $key=>$u): ?><?php ++$i;?><?php $mod = ($i % 2 )?><li style="display:none;">
                    <div class="userPic relative">
                    <?php echo getUserSpace($u["uid"],'','','{uavatar}') ?>
                      <?php if($mid >0 && $mid != $u['uid']) { ?>
                     <input name="followuid[]" type="checkbox" value="<?php echo ($u["uid"]); ?>" class="ckbox" />
                     <?php } ?>
                    </div>
                    <div id="star_<?php echo ($u["uid"]); ?>" class="title">
                      <?php echo getUserSpace($u["uid"],'','','{uname}') ?>
                      </div>
                    </li><?php endforeach; ?><?php endif; ?><?php else: echo "" ;?><?php endif; ?>
                <?php if(is_array($g["son_list"])): ?><?php $i = 0;?><?php $__LIST__ = $g["son_list"]?><?php if( count($__LIST__)==0 ) : echo "" ; ?><?php else: ?><?php foreach($__LIST__ as $key=>$s): ?><?php ++$i;?><?php $mod = ($i % 2 )?><?php if(is_array($s["user_list"])): ?><?php $i = 0;?><?php $__LIST__ = $s["user_list"]?><?php if( count($__LIST__)==0 ) : echo "" ; ?><?php else: ?><?php foreach($__LIST__ as $key=>$u): ?><?php ++$i;?><?php $mod = ($i % 2 )?><div style="width:16%; overflow:hidden;" class="left alC">
                      <div class="userPic" style="float:none; margin:0 auto"><?php echo getUserSpace($u["uid"],'','','{uavatar}') ?></div>
                      <div>
                        <?php if($mid >0 && $mid != $u['uid']) { ?>
                          <input name="followuid[]" type="checkbox" value="<?php echo ($u["uid"]); ?>" />
                          <?php } ?>
                        <?php echo getUserSpace($u["uid"],'','','{uname}') ?>
                        </div>
                      </div><?php endforeach; ?><?php endif; ?><?php else: echo "" ;?><?php endif; ?><?php endforeach; ?><?php endif; ?><?php else: echo "" ;?><?php endif; ?>
                <div class="c"></div>
                </ul>
              </form>
