...
<div class="content"><!-- 内容 begin  --> 
  <div class="main no_l">
    ...
    <div class="mainbox_R">
		...
		<?php if(is_array($list)): ?><?php $i = 0;?><?php $__LIST__ = $list?><?php if( count($__LIST__)==0 ) : echo "" ; ?><?php else: ?><?php foreach($__LIST__ as $key=>$vo): ?><?php ++$i;?><?php $mod = ($i % 2 )?><?php $css_class = $vo['document_id'] == $detail['document_id'] ? 'right_list_on' : 'right_list_def'; ?>
        <div class="<?php echo ($css_class); ?>"><div><a href="<?php echo U('home/Public/document',array('id'=>$vo['document_id']));?>"><?php echo ($vo["title"]); ?></a></div></div><?php endforeach; ?><?php endif; ?><?php else: echo "" ;?><?php endif; ?>
        </div>
    </div>
    <div class="mainbox_C">
    <div class="reg_box">
        <h2 class="cBlue" style="font-size:20px;"><strong><?php echo ($detail['title']); ?></strong></h2>
        <p style="padding: 5px 10px 30px;" class="cGray lineS_btm lh20 mLR15"><div class="ke-post" style="padding:15px"><?php echo ($detail['content']); ?></div></p>
    </div>
    <div class="c"></div>
    </div>
    </div>
