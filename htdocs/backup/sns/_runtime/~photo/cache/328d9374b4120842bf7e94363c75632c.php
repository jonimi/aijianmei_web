...
<div class="page_tit">
  <!-- page_title begin -->
  <ul class="tit_tab">
    <li><a href="<?php echo U('/Index/index');?>" <?php if(in_array((ACTION_NAME),array('index','all_photos','all_albums')) || !in_array((ACTION_NAME),array('all_photos','all_albums'))&&$uid!=$mid)echo 'class="on"'; ?>><?php echo ($ts['app']['app_alias']); ?>首页</a></li>
    <li><a href="<?php echo U('/Index/albums');?>" <?php if(!in_array((ACTION_NAME),array('index','all_photos','all_albums'))&&$uid==$mid)echo 'class="on"'; ?>>我的<?php echo ($ts['app']['app_alias']); ?></a></li>
  </ul>
  ...
  <div class="box_create"><a href="<?php echo U('photo/Upload/flash');?>" class="btn_photo_c"></a></div>
  <div class="right_box">
    ...
    <div class="tit"><a href="javascript:void(0);" onclick="ablumMove('down');" id="down" class="ico_down"></a></div>
      ...
      <?php if(is_array($IsHotList)): ?><?php $i = 0;?><?php $__LIST__ = $IsHotList?><?php if( count($__LIST__)==0 ) : echo "" ; ?><?php else: ?><?php foreach($__LIST__ as $key=>$li): ?><?php ++$i;?><?php $mod = ($i % 2 )?><li class="album_key" rel="<?php echo ($key); ?>" id="album_<?php echo ($key); ?>" style="display:none;">
          <div class="albums_img" ><a href="<?php echo U('/Index/album',array(uid=>$li['userId'],id=>$li['id']));?>"><img src="<?php echo (get_album_cover($li["id"],$li,66,66)); ?>"/></a></div>
          <div class="albums_info" style="width:50%">
            <dl>
              <dt><a href="<?php echo U('/Index/album',array(uid=>$li['userId'],id=>$li['id']));?>" class="OverH" style="width:8em; display:block;"><strong><?php echo ($li["name"]); ?></strong></a></dt>
              <dd>图片：(<?php echo ($li["photoCount"]); ?>)</dd>
              <dd><a href="<?php echo U('home/Space/index',array(uid=>$li[userId]));?>"><?php echo (getUserName($li["userId"])); ?></a></dd>
              <dd>时间:<?php echo (date('Y-m-d',$li["cTime"])); ?></dd>
            </dl>
          </div>
    ...
    case 'up':
      ...
      <div class="mainbox_appC">
        <div class="tab-menu"><!-- 切换标签 begin  -->
          <ul>
            <li><a href="<?php echo U('/Index/all_albums',array('order'=>'hot'));?>" <?php echo (empty($_GET['order']) || $_GET['order']=='hot')?'class="on"':NULL; ?>><span>热门<?php echo ($ts['app']['app_alias']); ?></span></a></li>
            <li><a href="<?php echo U('/Index/all_albums',array('order'=>'new'));?>" <?php echo ($_GET['order']=='new')?'class="on"':NULL; ?>><span>最新<?php echo ($ts['app']['app_alias']); ?></span></a></li>
			<li><a href="<?php echo U('/Index/all_albums',array('order'=>'following'));?>" <?php echo ($_GET['order']=='following')?'class="on"':NULL; ?>><span>我关注的人的<?php echo ($ts['app']['app_alias']); ?></span></a></li>
          </ul>
        </div>
        <!-- 切换标签 end  -->        
        <div class="photoBox">
          <div class="MenuSub">
			...
			<a href="<?php echo U('/Index/all_photos',array('order'=>'new'));?>">最新图片</a> | <a href="#" class="on">最新<?php echo ($ts['app']['app_alias']); ?></a>
		    <?php }elseif($_GET['order']=='following'){ ?>
			<a href="<?php echo U('/Index/all_photos',array('order'=>'following'));?>">我关注的人的图片</a> | <a href="#" class="on">我关注的人的<?php echo ($ts['app']['app_alias']); ?></a>
		    <?php }else{ ?>
			<a href="<?php echo U('/Index/all_photos',array('order'=>'hot'));?>">热门图片</a> | <a href="#" class="on">热门<?php echo ($ts['app']['app_alias']); ?></a>
		    <?php } ?>
           </div>
           <div class="falbumlist">
  <ul>
   <?php if(ACTION_NAME=='albums' && $weibo): ?>
      <li>
        <div class="box">
          <div class="c1"><span class="photo140"><a href="<?php echo U('photo/Index/weiboalbum',array(uid=>$weibo['userId'],id=>$li['id']));?>"><img src="<?php echo (get_album_cover($weibo["id"],$weibo)); ?>" alt="<?php echo ($weibo["name"]); ?>" /></a></span></div>
          <div class="c2">
            <dl>
              <dt>
              <span class="right">
                </span><a href="<?php echo U('photo/Index/weiboalbum',array(uid=>$weibo['userId'],id=>$weibo['id']));?>" class="OverH" alt="<?php echo ($weibo["name"]); ?>"><strong><?php echo ($weibo["name"]); ?></strong></a></dt>
                <dd>图片：<?php echo ($weibo["photoCount"]); ?></dd>
				<?php if( $mid != $weibo['userId']): ?><dd>作者：<a href="<?php echo U('home/Space/index',array(uid=>$weibo[userId]));?>"><?php echo (getUserName($weibo["userId"])); ?></a></dd><?php endif; ?>
				<dd><?php echo (friendlyDate($weibo["mTime"])); ?>更新</dd>
            </dl>
          </div>
        </div>
      </li>
	<?php endif; ?>
	<?php if(is_array($data["data"])): ?><?php $i = 0;?><?php $__LIST__ = $data["data"]?><?php if( count($__LIST__)==0 ) : echo "" ; ?><?php else: ?><?php foreach($__LIST__ as $key=>$li): ?><?php ++$i;?><?php $mod = ($i % 2 )?><li>
        <div class="box">
          <div class="c1"><span class="photo140"><a href="<?php echo U('/Index/album',array(uid=>$li['userId'],id=>$li['id']));?>"><img src="<?php echo (get_album_cover($li["id"],$li)); ?>" alt="<?php echo ($li["name"]); ?>" /></a></span></div>
          <div class="c2">
            <dl>
              <dt> <span class="right"></span><a href="<?php echo U('/Index/album',array(uid=>$li['userId'],id=>$li['id']));?>" class="OverH" alt="<?php echo ($li["name"]); ?>"><strong><?php echo ($li["name"]); ?></strong></a></dt>
              <dd>图片：<?php echo ($li["photoCount"]); ?></dd>
              <?php if( $mid != $li['userId']): ?><dd>作者：<a href="<?php echo U('home/Space/index',array(uid=>$li[userId]));?>"><?php echo (getUserName($li["userId"])); ?></a></dd><?php endif; ?>
              <dd><?php echo (friendlyDate($li["mTime"])); ?>更新</dd>
              <?php if(in_array($li['privacy'],array('2','3','4'))&&$mid!=$uid){ ?><dd>
                
                <img src="../Public/images/photo_ico_bgk.gif" width="13" height="12" />&nbsp;
                
                </dd><?php } ?>
              <dd><?php if( $mid == $li['userId'] ): ?><a href="<?php echo U('/Manage/album_edit',array(uid=>$li[userId],id=>$li['id']));?>">编辑该<?php echo ($ts['app']['app_alias']); ?></a><?php endif; ?></dd>
            </dl>
          </div>
        </div>
      </li><?php endforeach; ?><?php endif; ?><?php else: echo "" ;?><?php endif; ?>
...
<div class="page"><?php echo ($data['html']); ?></div>
          <!-- falbumlist end  --> 
        </div>
        <div class="c"></div>
      </div>
